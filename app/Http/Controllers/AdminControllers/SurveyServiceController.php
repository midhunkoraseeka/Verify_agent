...
class SurveyServiceController extends Controller
{
    ...
    {
        $request->validate([
            'service_name' => 'required|string|max:255|unique:rl_survey_services_tbl,service_name|regex:/^(?=.*[a-zA-Z])[a-zA-Z0-9\s\-]+$/',
        ], [
    ...
    {
        $request->validate([
            'service_name' => 'required|string|max:255|unique:rl_survey_services_tbl,service_name,' . $id . '|regex:/^(?=.*[a-zA-Z])[a-zA-Z0-9\s\-]+$/',

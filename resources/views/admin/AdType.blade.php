    ...
    <div class="page-head">
        ...
        <div class="action-group" style="display: flex; align-items: center; gap: 12px;">
            <div class="search-wrapper" style="position: relative;">
                ...
                <input type="text" id="adTypeSearch" placeholder="Search ad type..."
            ...
            <button type="button" class="btn-action btn-manage" data-bs-toggle="modal" data-bs-target="#addAdTypeModal">
                + Add Ad Type
            </button>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr data-name="{{ strtolower($type->ad_type_name) }}">
                        ...
                        <td class="text-end">
                            <div class="action-cell justify-content-end">
                                <button type="button" class="action-btn btn-edit me-2"
                                    ...
                                    data-bs-target="#editAdTypeModal{{ $type->id }}">Edit</button>
                                <a href="{{ route('deleteAdType', $type->id) }}"
                                ...
                                <form action="{{ route('updateAdType', $type->id) }}" method="POST">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="ad_type_name" class="form-control"
                                                value="{{ $type->ad_type_name }}" required
                                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                                            <span class="text-danger small ad-type-error" style="display:none;">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('storeAdType') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Add New Ad Type</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <label class="form-label">Ad Type Name <span class="req">*</span></label>
...
<script>
    // ── Search filter ──────────────────────────────────────────────
    document.getElementById('adTypeSearch').addEventListener('input', function () {
    ...
    document.addEventListener('submit', function (e) {
        const form  = e.target;
        const input = form.querySelector('[name="ad_type_name"]');
        if (input) {
            const isValid = validateAdType(input);

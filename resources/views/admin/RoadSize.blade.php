    ...
    <div class="page-head">
        ...
        <div class="action-group" style="display: flex; align-items: center; gap: 12px;">
            <div class="search-wrapper" style="position: relative;">
                ...
                <input type="text" id="roadSearch" placeholder="Search road size..."
            ...
            <button type="button" class="btn-action btn-manage" data-bs-toggle="modal" data-bs-target="#addRoadModal">
                + Add Road Size
            </button>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr data-name="{{ strtolower($road->road_size_name) }}">
                        ...
                        <td class="text-end">
                            <div class="action-cell justify-content-end">
                                <button type="button" class="action-btn btn-edit me-2"
                                    ...
                                    data-bs-target="#editRoadModal{{ $road->id }}">Edit</button>
                                <a href="{{ route('deleteRoadSize', $road->id) }}"
                            ...
                            <div class="modal-content text-start">
                                <form action="{{ route('updateRoadSize', $road->id) }}" method="POST">
                                    ...
                                    <div class="modal-header">
                                        <h5 class="modal-title">Edit Road Size</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label class="form-label">Road Size Name <span class="req">*</span></label>
                                            <input type="text" name="road_size_name" class="form-control"
                                                value="{{ $road->road_size_name }}" required
                                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                                            <span class="text-danger small road-error" style="display:none;">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('storeRoadSize') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Add New Road Size</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <label class="form-label">Road Size Name <span class="req">*</span></label>
...
<script>
    ...
    // ── Form validation ────────────────────────────────────────────
    function validateRoadSize(input) {
    ...
    document.addEventListener('submit', function (e) {
        const form  = e.target;
        const input = form.querySelector('[name="road_size_name"]');
        if (input) {
            const isValid = validateRoadSize(input);

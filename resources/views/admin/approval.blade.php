    ...
    <div class="page-head">
        ...
        <div class="action-group" style="display: flex; align-items: center; gap: 12px;">
            <div class="search-wrapper" style="position: relative;">
                ...
                <input type="text" id="approvalSearch" placeholder="Search approval..."
            ...
            <button type="button" class="btn-action btn-manage" data-bs-toggle="modal" data-bs-target="#addApprovalModal">
                + Add Approval
            </button>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr data-name="{{ strtolower($approval->approval_name) }}">
                        ...
                        <td class="text-end">
                            <div class="action-cell justify-content-end">
                                <button type="button" class="action-btn btn-edit me-2"
                                    ...
                                    data-bs-target="#editApprovalModal{{ $approval->id }}">Edit</button>
                                <a href="{{ route('deleteApproval', $approval->id) }}"
                                ...
                                <form action="{{ route('updateApproval', $approval->id) }}" method="POST">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="approval_name" class="form-control"
                                                value="{{ $approval->approval_name }}" required
                                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                                            <span class="text-danger small approval-error" style="display:none;">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('storeApproval') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Add New Approval</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <label class="form-label">Approval Name <span class="req">*</span></label>
                    <input type="text" name="approval_name" class="form-control"
                        placeholder="e.g. HMDA, DTCP2"
                        oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                    <span class="text-danger small approval-error" style="display:none;">
                        Must contain at least letters. Numbers, hyphens and spaces are optional.
                    </span>
...
<script>
    ...
    document.addEventListener('submit', function (e) {
        const form  = e.target;
        const input = form.querySelector('[name="approval_name"]');
        if (input) {
            const isValid = validateApproval(input);

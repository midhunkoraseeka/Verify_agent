...
<body>
        ...
        <div class="page-head">
            ...
            <div class="action-group" style="display: flex; align-items: center; gap: 12px;">
                <div class="search-wrapper" style="position: relative;">
                    ...
                    <input type="text" id="facingSearch" placeholder="Search facing..."
                ...
                <table class="table">
                    ...
                    <tbody>
                        @forelse($facings as $facing)
                            <tr data-name="{{ strtolower($facing->facing_name) }}">
                            ...
                            {{-- DYNAMIC EDIT MODAL --}}
                            <div class="modal fade" id="editFacingModal{{ $facing->id }}" tabindex="-1"
                                aria-hidden="true">
                                    ...
                                    <div class="modal-content text-start">
                                        ...
                                        <form action="{{ route('updateFacing', $facing->id) }}" method="POST">
                                            ...
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                    <label class="form-label">Facing Name <span
                                                            class="req">*</span></label>
                                                    <input type="text" name="facing_name" class="form-control"
                                                        value="{{ $facing->facing_name }}" required
                                                        oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                                                    <span class="text-danger small facing-error" style="display:none;">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <select name="status" class="form-select">
                                                        <option value="1"
                                                            {{ $facing->status == 1 ? 'selected' : '' }}>Active
                                                        </option>
                                                        <option value="0"
                                                            {{ $facing->status == 0 ? 'selected' : '' }}>Inactive
                                                        </option>
                                                    </select>
                                                </div>
                                            </div>
                ...
                <form action="{{ route('storeFacing') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="facing_name" class="form-control"
                                placeholder="e.g. East, North-West"
                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                            <span class="text-danger small facing-error" style="display:none;">
                               Should Contain atleast letters.
                            </span>
    ...
    <script>
        // ── Search filter ──────────────────────────────────────────────
        document.getElementById('facingSearch').addEventListener('input', function () {
        ...
        document.addEventListener('submit', function (e) {
            const form  = e.target;
            const input = form.querySelector('[name="facing_name"]');
            if (input) {
                const isValid = validateFacing(input);

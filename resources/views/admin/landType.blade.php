...
<div class="content">
    <div class="page-head">
        <h1 class="page-title">Land Type Master Data</h1>
        <div class="action-group" style="display: flex; align-items: center; gap: 12px;">
            <div class="search-wrapper" style="position: relative;">
                ...
                <input type="text" id="landTypeSearch" placeholder="Search land type..."
            ...
            <button type="button" class="btn-action btn-manage" data-bs-toggle="modal" data-bs-target="#addLandTypeModal">
                + Add Land Type
            </button>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr data-name="{{ strtolower($type->land_type_name) }}">
                        <td>{{ $loop->iteration }}</td>
                        <td><strong>{{ $type->land_type_name }}</strong></td>
                        <td>
                        ...
                        <td class="text-end">
                            <div class="action-cell justify-content-end">
                                <button type="button" class="action-btn btn-edit me-2"
                                    ...
                                    data-bs-target="#editLandTypeModal{{ $type->id }}">Edit</button>
                                <a href="{{ route('deleteLandType', $type->id) }}"
                    ...
                    {{-- Edit Modal --}}
                    <div class="modal fade" id="editLandTypeModal{{ $type->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content text-start">
                                <form action="{{ route('updateLandType', $type->id) }}" method="POST">
                                    @csrf
                                    <div class="modal-header">
                                        <h5 class="modal-title">Edit Land Type</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label class="form-label">Land Type Name <span class="req">*</span></label>
                                            <input type="text" name="land_type_name" class="form-control"
                                                value="{{ $type->land_type_name }}" 
                                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                                            <span class="text-danger small land-type-error" style="display:none;">
...
{{-- Add Modal --}}
<div class="modal fade" id="addLandTypeModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('storeLandType') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Add New Land Type</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <label class="form-label">Land Type Name <span class="req">*</span></label>
                    <input type="text" name="land_type_name" class="form-control"
                        placeholder="e.g. Residential, Agricultural" required
                        oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                    <span class="text-danger small land-type-error" style="display:none;">
                        Must contain at least letters. Numbers, hyphens and spaces are optional.
                    </span>
...
<script>
    // ── Search filter ──────────────────────────────────────────────
    document.getElementById('landTypeSearch').addEventListener('input', function () {
    ...
    // ── Form validation ────────────────────────────────────────────
    function validateLandType(input) {
    ...
    document.addEventListener('submit', function (e) {
        const form  = e.target;
        const input = form.querySelector('[name="land_type_name"]');
        if (input) {
            const isValid = validateLandType(input);

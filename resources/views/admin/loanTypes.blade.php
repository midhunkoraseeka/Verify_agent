    ...
    <div class="page-head">
        ...
        <div class="action-group" style="display: flex; align-items: center; gap: 12px;">
            <div class="search-wrapper" style="position: relative;">
                ...
                <input type="text" id="loanTypeSearch" placeholder="Search loan type..."
            ...
            <button type="button" class="btn-action btn-manage" data-bs-toggle="modal" data-bs-target="#addLoanTypeModal">
                + Add Loan Type
            </button>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr data-name="{{ strtolower($type->loan_type_name) }}">
                        ...
                        <td class="text-end">
                            <div class="action-cell justify-content-end">
                                <button type="button" class="action-btn btn-edit me-2"
                                    ...
                                    data-bs-target="#editLoanTypeModal{{ $type->id }}">Edit</button>
                                <a href="{{ route('deleteLoanType', $type->id) }}"
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="loan_type_name" class="form-control"
                                                value="{{ $type->loan_type_name }}" required
                                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                                            <span class="text-danger small loan-type-error" style="display:none;">
        ...
        <div class="modal-content">
            <form action="{{ route('storeLoanType') }}" method="POST">
                ...
                <div class="modal-body">
                    <label class="form-label">Loan Type Name <span class="req">*</span></label>
                    <input type="text" name="loan_type_name" class="form-control"
                        placeholder="e.g. House Loan, Personal Loan"
                        oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                    <span class="text-danger small loan-type-error" style="display:none;">
                        Must contain at least letters. Numbers, hyphens and spaces are optional.
                    </span>
...
<script>
    // ── Search filter ──────────────────────────────────────────────
    document.getElementById('loanTypeSearch').addEventListener('input', function () {
    ...
    // ── Form validation ────────────────────────────────────────────
    function validateLoanType(input) {
    ...
    document.addEventListener('submit', function (e) {
        const form  = e.target;
        const input = form.querySelector('[name="loan_type_name"]');
        if (input) {
            const isValid = validateLoanType(input);

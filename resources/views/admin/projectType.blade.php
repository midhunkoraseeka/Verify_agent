    ...
    <div class="page-head">
        ...
        <div class="action-group" style="display: flex; align-items: center; gap: 12px;">
            <div class="search-wrapper" style="position: relative;">
                ...
                <input type="text" id="projectTypeSearch" placeholder="Search project type..."
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr data-name="{{ strtolower($type->project_type_name) }}">
                                ...
                                <form action="{{ route('updateProjectType', $type->id) }}" method="POST">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="project_type_name" class="form-control"
                                                value="{{ $type->project_type_name }}" required
                                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                                            <span class="text-danger small project-type-error" style="display:none;">
            ...
            <form action="{{ route('storeProjectType') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Project Type Name <span class="req">*</span></label>
                        <input type="text" name="project_type_name" class="form-control"
                            placeholder="e.g. Gated Community, Open Plots" 
                            oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                        <span class="text-danger small project-type-error" style="display:none;">
                            Must contain at least letters. Numbers, hyphens and spaces are optional.
                        </span>
...
<script>
    // ── Search filter ──────────────────────────────────────────────
    document.getElementById('projectTypeSearch').addEventListener('input', function () {
    ...
    document.addEventListener('submit', function (e) {
        const form  = e.target;
        const input = form.querySelector('[name="project_type_name"]');
        if (input) {
            const isValid = validateProjectType(input);

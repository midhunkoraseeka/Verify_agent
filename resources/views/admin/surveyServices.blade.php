...
<head>
    ...
    <title>Manage Survey Services</title>
...
<div class="content">
    <div class="page-head">
        <h1 class="page-title">Survey Service Master Data</h1>
        <div class="action-group" style="display: flex; align-items: center; gap: 12px;">
            <div class="search-wrapper" style="position: relative;">
                ...
                <input type="text" id="surveyServiceSearch" placeholder="Search survey service..."
            ...
            <button type="button" class="btn-action btn-manage" data-bs-toggle="modal" data-bs-target="#addSurveyModal">
                + Add Service
            </button>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr data-name="{{ strtolower($service->service_name) }}">
                        ...
                        <td class="text-end">
                            <div class="action-cell justify-content-end">
                                <button type="button" class="action-btn btn-edit me-2"
                                    ...
                                    data-bs-target="#editSurveyModal{{ $service->id }}">Edit</button>
                                <a href="{{ route('deleteSurveyService', $service->id) }}"
                    ...
                    {{-- Edit Modal --}}
                    <div class="modal fade" id="editSurveyModal{{ $service->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content text-start">
                                <form action="{{ route('updateSurveyService', $service->id) }}" method="POST">
                                    @csrf
                                    <div class="modal-header">
                                        <h5 class="modal-title">Edit Survey Service</h5>
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <span class="text-danger small survey-service-error" style="display:none;">
...
<div class="modal fade" id="addSurveyModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('storeSurveyService') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Add New Survey Service</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <label class="form-label">Service Name <span class="req">*</span></label>
                    <input type="text" name="service_name" class="form-control"
                        placeholder="e.g. Land Survey, Digital Mapping"
                        oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s\-]/g, '');">
                    <span class="text-danger small survey-service-error" style="display:none;">
                        Must contain at least letters. Numbers, hyphens and spaces are optional.
                    </span>
...
<script>
    // ── Search filter ──────────────────────────────────────────────
    document.getElementById('surveyServiceSearch').addEventListener('input', function () {
    ...
    // ── Form validation ────────────────────────────────────────────
    function validateSurveyService(input) {
    ...
    document.addEventListener('submit', function (e) {
        const form  = e.target;
        const input = form.querySelector('[name="service_name"]');
        if (input) {
            const isValid = validateSurveyService(input);
